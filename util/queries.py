...
async def statboard_raid_active(config, area):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_raid_active.execute(f"select count(id), ifnull(sum(raid_level = 1), 0), ifnull(sum(raid_level = 2),0), ifnull(sum(raid_level = 3),0), ifnull(sum(raid_level = 4),0), ifnull(sum(raid_level = 5),0), ifnull(sum(raid_level = 6),0), ifnull(sum(raid_level = 7),0), ifnull(sum(raid_level = 8),0), ifnull(sum(raid_level = 9),0) from gym where raid_battle_timestamp < UNIX_TIMESTAMP() AND raid_end_timestamp >= UNIX_TIMESTAMP() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_egg_active(config, area):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_egg_active.execute(f"select count(id), ifnull(sum(raid_level = 1), 0), ifnull(sum(raid_level = 2),0), ifnull(sum(raid_level = 3),0), ifnull(sum(raid_level = 4),0), ifnull(sum(raid_level = 5),0), ifnull(sum(raid_level = 6),0), ifnull(sum(raid_level = 7),0), ifnull(sum(raid_level = 8),0), ifnull(sum(raid_level = 9),0) from gym WHERE raid_battle_timestamp >= UNIX_TIMESTAMP() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_lure_active(config, area):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_lure_active.execute(f"select count(id), ifnull(sum(lure_id = 501), 0), ifnull(sum(lure_id = 502), 0), ifnull(sum(lure_id = 503), 0), ifnull(sum(lure_id = 504), 0), ifnull(sum(lure_id = 505), 0) from pokestop where lure_expire_timestamp > UNIX_TIMESTAMP() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
